...
4. [Система теплого пола](#система-теплого-пола)
5. [Кухня](#кухня)
...
- **Общие требования и принципы:**
  ...
  - Нормы притока на человека: 60 м³/ч·чел (бодрствование), 30 м³/ч·чел (сон); для расчётов среднего режима допускается 50 м³/ч·чел (см. `supply_air.md`).
...
- **Мониторинг и датчики:**
  - **Контроль работы вентиляторов:** Датчики дифференциального либо абсолютного давления (4–20 мА) PT‑506 как датчик абсолютного
  ...
  - **Датчики CO₂:** ZigBee NDIR датчики. Устанавливаются в месте наиболее вероятного нахождения людей. Спальня - возле кровати, кабинет - возле стола и т.д.
  - **Датчики влажности/температуры:** устанавливается отдельно от CO₂ датчика т.к. NDIR вносит погрешности в измерение температуры ввиду своего самонагрева
...
- **Защита от превышения давления** 
  - На входе системы отопления стоят компенсаторы гидроудара.
...
В кухне над вытяжкой, помимо стандартной вытяжки, устанавливается вентилятор Noctua Industrial PPC 24 В 140mm с управлением по PWM через модуль [JPF 4816](https://github.com/AlexMKX/modbus/tree/main/JUF4802) либо EC - вентилятор с 0–10 В управлением : [задача на тестирование](https://task.hacksaw.team/browse/AC-7). 
